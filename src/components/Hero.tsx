...
<div
className="orb absolute w-96 h-96 bg-cyan-400 -top-20 -right-20"
style={{ animationDelay: '0s' }}
/>
<div
className="orb absolute w-80 h-80 bg-blue-400 bottom-20 -left-20"
style={{ animationDelay: '2s' }}
/>
<div
className="orb absolute w-64 h-64 bg-indigo-300 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
    ...
    >
      ...
      <div
        className={`text-white space-y-6 ${
          isRTL ? 'lg:col-start-2 text-right' : ''
        }`}
      >
        {/* Badge */}
        ...
        <div className="space-y-2">
          ...
          >
            {['HL7', 'ASTM', 'Middleware'].map((tag) => (
              <span
                key={tag}
                className="px-3 py-1 bg-white/10 border border-white/20 rounded-full text-sm text-white/90 font-medium"
              >
                {tag}
              </span>
            ))}
      ...
      >
        <div className="relative">
          ...
          <div className="relative w-64 h-64 sm:w-80 sm:h-80 lg:w-96 lg:h-96 rounded-full overflow-hidden border-4 border-white/20 shadow-2xl bg-gradient-to-br from-blue-700 to-cyan-600">
            <img
              src="/profile.jpg"
              alt="Mohmed Abdalazim"
